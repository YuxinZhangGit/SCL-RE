...
from www import prediction
from www.entities import Document, Dataset, EntityType
from www.input_reader import BaseInputReader
from www.opt import jinja2
...
class Evaluator:
    def __init__(self, dataset: Dataset, input_reader: BaseInputReader, text_encoder: BertTokenizer,
        ...
        # relations
        self._gt_relations = []  # label
        self._pred_relations = []  # prediction

        # entities
        self._gt_entities = []  # label
    ...
    # 评估预测结果的质量 对所有类型进行划分 分类统计
    # :param gt: label数据
    # :param pred: 预测数据
    # :param print_results: 是否打印结果的布尔变量
    # :return: 包含所有结果的list
        assert len(gt) == len(pred)
