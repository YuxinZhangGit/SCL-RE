        ...
        with tqdm(total=len(self.valid_dataloader), ncols=80) as tqbar:
            with torch.no_grad():
                while True:
                    ...
                    metrics, _ = self.batch_metrics_func(labels, preds, metrics, self)
            ...
            for k, v in metrics_result.items():
                self.logger.info("epoch {0} : valid {1}\t{2}".format(epoch, k, v))
            self.logger.info("label_precision_num: {0}, label_recall_num: {1}".format(len(metrics['label_precision']),len(metrics['label_recall'])))
    ...
    def train_epoch(self, epoch: int) -> Dict[str, int]:
        ...
        with tqdm(total=len(self.training_dataloader), ncols=80) as tqbar:
            ...
            while True:
                ...
                metrics, batch_matrics = self.batch_metrics_func(labels, preds, metrics, self)
                total_loss+=loss.item()
            ...
            try:
                ...
                if (epoch+1) % self.valid_step == 0 and self.valid_dataset is not None:
                    ...
                    with open(os.path.join(self.output_dir, "metrics{0}.json".format(epoch)), "w") as f:
                        json.dump(self.epoch_metrics[-1], f,indent=4,ensure_ascii=False,default=str)
                if self.save_model:
