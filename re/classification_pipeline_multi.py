...
from typing import *
from classification_model import NewPromptModel
...
class NewPromptPipeline(object):
    ...
    def _predict_batch(self, textIds: List[str], texts: List[str],companies):
        ...
        ##################判断其他关系######################
        pred2=torch.topk((decoder_output+vocab_masks_repeated),2)[1]
        pred2=pred2[:,:,1].reshape([1,len(mask4mask)])

        decoder_softmax=F.softmax(decoder_output,dim=2)
        conf=0.0
        for mask_index in range(mask_start_end[0][0],mask_start_end[0][1]+1):
            conf+=decoder_softmax[0][mask_index][pred2[0][mask_index]]
        
        if conf > 1.4:
            preds_tokens2=[]
            pred_mask_token_ids2=pred2[0][mask_start_end[0][0]:mask_start_end[0][1]+1]
            pred_token2="".join(self.tokenizer.decode(pred_mask_token_ids2).split(" ")).replace(self.tokenizer.pad_token,"")
            preds_tokens2.append(pred_token2)
            preds2=[]
            preds2.append(self._get_event_type(preds_tokens2[0]))
            for i in preds2:
                res.append(self.id2event[i])
        ##################################################
        return res
...
if __name__ == "__main__":
    ...
    key_counts=[]
    pipeline=NewPromptPipeline(event2id,"/data/zyx/ccks2/new_prompt_output_final2/epoch7.pt",3,"nghuyong/ernie-1.0",1)
    data = []
